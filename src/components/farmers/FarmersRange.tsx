...
import { farmersProductRange } from "@/lib/farmers";

export function FarmersRange() {
  return (
    <section
      aria-labelledby="farmers-range-heading"
      className="bg-white py-20 sm:py-24 lg:py-28"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Reveal>
          <SectionHeading
            id="farmers-range-heading"
        ...
        <Stagger className="mt-10 grid auto-rows-fr items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-5">
          {farmersProductRange.map((item, index) => (
            <StaggerItem key={item.title} className="h-full">
              <Link href={item.href} className="block h-full">
                <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-cream-2 shadow-sm transition hover:-translate-y-0.5 hover:border-gold/40 hover:shadow-md">
